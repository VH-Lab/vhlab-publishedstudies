...
oridirfigure = figure('tag','SurrOriDir_ResponsesAndStats');
...
for p = 1:P,
	...
	for i=1:length(myinds{p}),
		if ~isempty(cellinfo(myinds{p}(i)).surrori_info.sig_ori) & ~isempty(cellinfo(myinds{p}(i)).ori_info.sig_ori),
			if (cellinfo(myinds{p}(i)).ori_info.sig_ori<0.05) & (max(cellinfo(myinds{p}(i)).surrori_info.fit(2,:))>-Inf),
	...
	for i=1:length(mygoodcells),
		fit_new = cellinfo(mygoodcells(i)).surrori_info.fit;
		fit_center = cellinfo(mygoodcells(i)).surrori_info.fit;
		fit_new(2,:) = rotatedircurve(fit_new(1,:),fit_new(2,:),90,fit_center(2,:));
	...
	cv_ind{p} = [];
	for i=1:length(mygoodcells),
		dir_ind{p}(end+1) = cellinfo(mygoodcells(i)).surrori_info.di;
		cv_ind{p}(end+1) = cellinfo(mygoodcells(i)).surrori_info.cv;
	end;
