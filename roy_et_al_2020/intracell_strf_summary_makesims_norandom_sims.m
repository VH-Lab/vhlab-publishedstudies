function sim_resps = intracell_strf_summary_makesims_norandom_sims(oristruct, vmf_model, simpleflag, average_models)
%
...
for n=1:numel(y),
	...
	sim_data_resp_n = [];
	for k=1:1,
		% draw a cell not at random for its voltages
		cell_v = n;
		...
		[di_here,resp_p,resp_n] = simulate_prefnull(voltage_here_pref,voltage_here_null,voltage_here_control, ...
			average_models.slope{1}, average_models.offset{1}, average_models.threshold{1}, average_models.exponent{1});
		sim_data_di(end+1) = di_here;
		sim_data_resp_p(end+1) = resp_p;
		sim_data_resp_n(end+1) = resp_n;
	end;
	sim_resps{young}{young}(end+1,:) = [mean(sim_data_di) mean(sim_data_resp_p) mean(sim_data_resp_n)];
end;
...
for n=1:numel(y),
	...
	for k=1:1, 
		...
		voltage_here_control = oristruct(y(cell_v)).out_control;
		[di_here,resp_p,resp_n] = simulate_prefnull(voltage_here_pref, voltage_here_null, voltage_here_control, ...
			average_models.slope{2}, average_models.offset{2}, average_models.threshold{2}, average_models.exponent{2});
		sim_data_di(end+1) = di_here;
		sim_data_resp_p(end+1) = resp_p;
		sim_data_resp_n(end+1) = resp_n;
	end;
	sim_resps{young}{old}(end+1,:) = [mean(sim_data_di) mean(sim_data_resp_p) mean(sim_data_resp_n)];
end;
...
for n=1:numel(o),
	...
	sim_resps{old}{young}(end+1,:) = [mean(sim_data_di) mean(sim_data_resp_p) mean(sim_data_resp_n)];
end;
...
for n=1:numel(o),
	...
	sim_data_resp_n = [];
	for k=1:1,
		% draw a cell at non-random for its voltages
		cell_v = n; 
	...
	sim_resps{old}{old}(end+1,:) = [mean(sim_data_di) mean(sim_data_resp_p) mean(sim_data_resp_n)];
end;

function [di, resp_p, resp_n] = simulate_prefnull(pref_struct, null_struct, control_struct, slope, offset, threshold, exponent)
