...
for age1=1:2,
	for age2=1:2,
		...
		[di_line_x{age1}{age2}, di_line_y{age1}{age2}] = vlt.plot.hist2line(bin_edges, counts);
		counts_resp = histc(sim_resps{age1}{age2}(:,pref),bin_edges_resp);
		[di_line_x_resp{age1}{age2}, di_line_y_resp{age1}{age2}] = vlt.plot.hist2line(bin_edges_resp, counts_resp);
...
plot([1 1],[mean(out_young(:,1))+[-1 1]*vlt.data.nanstderr(out_young(:,1))],'k-');
plot(1+0.1*randn(size(out_young(:,1))),out_young(:,1),'ko');

plot(7+[0.5 0.5 1.5 1.5],[0 mean(out_old(:,1)) mean(out_old(:,1)) 0],'k-');
plot(7+[1 1],[mean(out_old(:,1))+[-1 1]*vlt.data.nanstderr(out_old(:,1))],'k-');
...
plot([1 1],[nanmean(out_young(:,4))+[-1 1]*vlt.data.nanstderr(out_young(:,4))],'k-');
plot(1+0.1*randn(size(out_young(:,4))),out_young(:,4),'ko');

plot(7+[0.5 0.5 1.5 1.5],[0 nanmean(out_old(:,4)) nanmean(out_old(:,4)) 0],'k-');
plot(7+[1 1],[nanmean(out_old(:,4))+[-1 1]*vlt.data.nanstderr(out_old(:,4))],'k-');
