function sim_resps = intracell_strf_summary_makesims2(oristruct, vmf_model, simpleflag, num_cells_per_sim, numsims)
%
...
for n=1:numel(o),
	...
	for k=1:1,
		...
		cell_v = n; 
		% draw same cell at non-random for its gains
		cell_g = n; 
...
for i=1:numel(pref_struct.TT),
	if ~any(isnan(pref_struct.raw_data{i})),
		R = sum(pref_struct.spike_data{i})/(pref_struct.TT{i}(end)-pref_struct.TT{i}(1));
...
for i=1:numel(null_struct.TT),
	if ~any(isnan(null_struct.raw_data{i})),
		R = sum(null_struct.spike_data{i})/(null_struct.TT{i}(end)-null_struct.TT{i}(1));
...
for i=1:numel(control_struct.TT),
	if ~any(isnan(control_struct.raw_data{i})),
		R = sum(control_struct.spike_data{i})/(control_struct.TT{i}(end)-control_struct.TT{i}(1));
