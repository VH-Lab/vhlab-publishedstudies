...
v_plot_lowsample = vlt.data.findclosest(v_plot,v_interp(1));
v_plot_highsample = vlt.data.findclosest(v_plot,v_interp(end));
...
for i=1:numel(curvestruct),
	...
	vlt.plot.supersubplot(fig,Ny,Nx,i);
	...
	if 1,
		...
		if ~strcmp(curvestruct(i).fit(2).fittype,'vlt.fit.linepowerthreshold'),
			error(['We were trying to access vlt.fit.linepowerthreshold, got ' curvestruct(i).fit(2).fittype ' instead.']);
		end;
	end;

	h=vlt.plot.myerrorbar(curvestruct(i).Xn,curvestruct(i).Yn,curvestruct(i).Yint,curvestruct(i).Yint);
	...
	if numel(find(curvestruct(i).firingrate_observations>0))>15, % exclude cells with no or almost no spikes
		...
		n_summary{1+1-curvestruct(i).young}(:,end+1) = vlt.math.slidingwindowfunc(curvestruct(i).voltage_observations,...
...
for i=1:2,
    y = vlt.data.colvec(nanmean(v_summary{i},2));
    w = vlt.data.colvec(nanmean(n_summary{i}([v_plot_lowsample:v_plot_highsample]'),2));
    w(find(isnan(y))) = 0;
    y(find(isnan(y))) = 0; 
	[slope{i},offset{i},threshold{i},exponent{i}, curve{i},gof{i},fitinfo{i}] = ...
		vlt.fit.linepowerthresholdfit(v_interp(:), y, ...
...
for i=1:2,
	h=vlt.plot.myerrorbar(v_interp(:), nanmean(v_summary{i},2), vlt.data.nanstderr(v_summary{i}')', vlt.data.nanstderr(v_summary{i}')');
	hold on;
	set(h,'color',c(i,:));
	if showfit,
		delete(h(2));
...
[adjusted_p,raw_p] = vlt.stats.twocurves_bhfdr(v_summary{1},v_summary{2});
...
for i=1:2,
	h=vlt.plot.myerrorbar(v_plot(:), nanmean(n_summary{i},2), vlt.data.nanstderr(n_summary{i}')', vlt.data.nanstderr(n_summary{i}')');
	hold on;
	set(h,'color',c(i,:));
	delete(h(2));
end;
...
out = vlt.data.var2struct('slope','offset','threshold','exponent','curve','gof','fitinfo');
