function table = readchr2positionstimdata(ds, cell, cellname, varargin)
% READCHR2POSITIONSTIMDATA - Read projector position stim data into a table
%
%  TABLE = READCHR2POSITIONSTIMDATA(DS, CELL, CELLNAME)
%
...
%     The columns are, respectively, Xposition, Yposition, radius, mean response, stderr response
%
...
for t=1:length(T),
	...
	for n=1:numStims(stims.saveScript),
        ...
        if ~isfield(parameters,'isblank'),
            ...
            table_entry(4) = mean(spike_counts);
            table_entry(5) = stderr(spike_counts(:));
