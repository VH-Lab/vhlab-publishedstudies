%
%  % control flags
perform_otcarandini_bootstrap = 0;
perform_vector_bootstrap = 1;
...
% (3) give 'expected response' an orientation-selective but not direction-selective response
%expected_response = [2 2 10 2 2 2 10 2],
...
for i_rep = 1:ncells
  ...
  if perform_vector_bootstrap,
	predictor = angles;
	trialdata = response_mtx;
	niter = 100;
	simdataflag = 0; % use bootstrap
	verbose = 1;
	extraparams = {trialdata,0};
	usefulldata = 1;

	[parammtx,datamtx,extraout] = resampparams_MC('mark_vector_wrap',predictor,trialdata,...
		niter,simdataflag,extraparams,verbose, usefulldata);

	vectorbootstrap_params{i_rep} = {parammtx, datamtx, extraout};

	vectorbootstrap_uncertainty(i_rep) = dpuncertainty(vectorbootstrap_params{i_rep}{1}(:,1));


  end; % perform_vector_bootstrap

  if perform_otcarandini_bootstrap,
  
	% Perform the ot_carandini bootstrap
	predictor = angles;
	trialdata = response_mtx;
	niter = 3;
	simdataflag = 0;
...
%   figure,hist(vectormags_scaled)
  ...
  end;  % perform_otcarandini_bootstrap
