function [prefdir p_dirselective p_reverse p_oriselective p_vectormag vectors vectormags vectormags_scaled] = calc_direction_stats_vectormethod_V2(angles,response_mtx,spontaneous,nrep)
% [prefdir p_dirselective p_reverse p_oriselective vectors vectormags] = calc_direction_stats_vectormethod(angles,response_mtx,spontaneous,nrep)
...
% SPONTANEOUS [OPTIONAL]: The spontaneous response. If not given, it will be taken as the minimum average response in response_mtx.
% NREP [OPTIONAL]: If you give a number here, the code will resample the data 'nrep' times, bascially simulating 'nrep' trials where each
% trial is a set of responses, one from each angle.

% ----Outputs----
% PREFDIR: The preferred direction inferred from the vector average of individual trials.
% P_DIRSELECTIVE: Uses Hotelling's T^2 test on direction vectors to get a p value for direction selectivity.
...
% P_ORISELECTIVE: Uses Hotelling's T^2 test on orientation vectors to get a p value for orientation selectivity.
% P_VECTORMAG: Performs a T test on vector magnitude.
% VECTORS: ntrials-by-2 matrix of direction vectors.
% VECTORMAGS: ntrials-length list of direction vector magnitudes.
% VECTORMAGS_SCALED: ntrials-length list of direction vector magnitudes rescaled by logit transformation.
%  --This is important only to observe the Gaussian distribution of vector magnitudes that is produced by this analysis.

% MM 11/14/12
% Updated 12/4/12 to include orientation test
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Perform the resampling, if specified

if(~isempty(nrep))
  rmtx_new = nan*ones(nrep,length(angles));
  for i_angle = 1:length(angles)
    r1 = response_mtx(:,i_angle);
    randind = ceil(rand(nrep,1)*length(r1));
    rmtx_new(:,i_angle) = response_mtx(randind,i_angle);
  end
  response_mtx = rmtx_new;
end
...
X = [x y];
[H,p_dirselective]=hotellingt2test(X,[0 0]);
...
p_reverse = normcdf(0,mean(vectormags_scaled),std(vectormags_scaled));

% Now do a T test on the vector magnitudes
[q p_vectormag] = ttest(vectormags_scaled);

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Calculate p value for orientation selectivity by calculating an orientation response vector on each trial and using Hotelling's T^2 test
% to measure significance

angles_polar = (angles/180)*2*pi;
...
% Do stats on this distribution to ask whether the cell is directionally-selective
X = [x y];
[H,p_oriselective]=hotellingt2test(X,[0 0]);
