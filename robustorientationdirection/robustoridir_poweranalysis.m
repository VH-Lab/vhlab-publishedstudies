...
measure,population_size,population_increment,
	...
	for r=1:reps,
		...
		for k=1:num_sims,
			...
			for i=1:population_size, % generate correct number of cells
				...
				if useori,
					[rsp_,rp_,op_,sigma_,rn_] = osi2oriparams(base,'sigma',sigma_);
				else,
					[rsp_,rp_,op_,sigma_,rn_] = dsi2dirparams(base,'sigma',sigma_);
				end;
				sigma = (generate_random_data(1,'gamma',3,6)+10)/1.18;
				if useori,
					[rsp,rp,op,sigma,rn] = osi2oriparams(base+change,'sigma',sigma);
				else,
					[rsp,rp,op,sigma,rn] = dsi2dirparams(base+change,'sigma',sigma);
				end;
				op_ = rand * 360;
				op  = rand * 360;
