...
if nargin==0, % open new figure and draw it
  ...
  switch thetag,
    ...
    case {'RC1CB','RC2CB','SFCB','SusTransLGNCB','ContrastCB',...
		  'Phase1CB','Phase2CB',...
          'Phase3CB','TFCB','OTCB','SpontaneousCB','ColorCB','ColorBRCB','ColorGRCB','ColorSelectCB','BlueConeCB','GreenConeCB','RodCB'},
       cases={'RC1CB','RC2CB','SFCB','SusTransLGNCB','ContrastCB',...
	      'Phase1CB','Phase2CB','Phase3CB','TFCB','OTCB','SpontaneousCB','ColorCB','ColorBRCB','ColorGRCB','ColorSelectCB','BlueConeCB','GreenConeCB','RodCB'};
       tests={'RC1TestEdit','RC2TestEdit','SFTestEdit','SusTransLGNEdit',...
	          'ContrastTestEdit','Phase1TestEdit',...
				...
				'SpontaneousTestEdit','ColorTestEdit','ColorBRTestEdit','ColorGRTestEdit','ColorSelectTestEdit','BlueConeTestEdit','GreenConeTestEdit','RodTestEdit'};
       ...
       asTests = {'SGS coarse tests','SGS fine tests','SF tests',...
			...
			'Phase2 tests', 'Phase3 tests','TF tests','OT tests',...
				'SpontaneousCB','ColorCB','ColorBRCB','ColorGRCB','ColorSelectCB','BlueConeCB','GreenConeCB','RodCB'};
	   asBestTests={'SGS coarse best test','SGS fine best test',...
			...
			'TF best test','OT best test','SpontaneousCB','ColorCB','ColorBRCB','ColorGRCB','ColorSelectCB','BlueConeCB','GreenConeCB','RodCB'};
    ...
    case {'AnalyzeOTBt','AnalyzeSFBt','AnalyzeTFBt','AnalyzeContrastBt', ...
			'AnalyzePhase1Bt','AnalyzePhase2Bt','AnalyzePhase3Bt','AnalyzeColorBt','AnalyzeColorBRBt','AnalyzeColorGRBt','AnalyzeColorSelectBt','AnalyzeBlueConeBt','AnalyzeGreenConeBt','AnalyzeRodBt'},
	   cases={'AnalyzeOTBt','AnalyzeSFBt','AnalyzeTFBt','AnalyzeContrastBt',...
	               'AnalyzePhase1Bt','AnalyzePhase2Bt','AnalyzePhase3Bt',...
				   	'AnalyzeColorBt','AnalyzeColorBRBt','AnalyzeColorGRBt','AnalyzeColorSelectBt','AnalyzeBlueConeBt','AnalyzeGreenConeBt','AnalyzeRodBt'};
	   testEditStrs = {'OTTestEdit','SFTestEdit','TFTestEdit',...
	   		'ContrastTestEdit',...
	   		'Phase1TestEdit','Phase2TestEdit','Phase3TestEdit','ColorTestEdit','ColorBRTestEdit','ColorGRTestEdit','ColorSelectTestEdit','BlueConeTestEdit','GreenConeTestEdit','RodTestEdit'};
	   ps={'angle','sFrequency','tFrequency','contrast','sPhaseShift',...
	   		'sPhaseShift','sPhaseShift','contrast','contrast','contrast','sFrequency','sFrequency','sFrequency','sFrequency'};
       ...
       if ~isempty(g),
		 ...
		 %for i=1:length(reflist),
			[s,c,data]=getcurrentstimcellinfo(thefig,cksds,g);
			if ~isempty(s),
				if strcmp(cases{ii},'AnalyzeColorBt')|strcmp(cases{ii},'AnalyzeColorBRBt')|strcmp(cases{ii},'AnalyzeColorGRBt'),%tie color changes to contrast
					do = getDisplayOrder(s.stimscript);
					for j=1:numStims(s.stimscript),
						p=getparameters(get(s.stimscript,j));
						p.contrast = (j-1)/12;
						s.stimscript = set(s.stimscript,periodicstim(p),j);
					end;
				...
				for j=1:length(c),
					...
					if isctx&get(ft(thefig,'AnalyzeCTXINTCB'),'value'),
						A.t0 = 0.002; A.t1 = 0.009; % spike removal params
	...
	case {'ColorRunBt','ColorBRRunBt','ColorGRRunBt'},
		  currDir = pwd;
		  path=fileparts(which('lgnctxexperpanel'));
		  cd(path);
		  load colorpairs_pi.mat -mat
		  colorpairs = colorpairsBG;  % unlabeled is BG
		  cd(currDir);
          ...
          if b,
			...
			[cr,dist,sr] = getscreentoolparams;
			COLOR_PS = stimscript(0);
			teStr = 'ColorTestEdit'; cbStr = 'ColorCB';
			if strcmp(thetag,'ColorBRRunBt'),
				teStr = 'ColorBRTestEdit';
				cbStr = 'ColorBRCB';
				colorpairs = colorpairsBR;
			elseif strcmp(thetag,'ColorGRRunBt'),
				teStr = 'ColorGRTestEdit';
				cbStr = 'ColorGRCB';
				colorpairs = colorpairsGR;
			end;
			for i=1:length(colorpairs)/2;
				p.chromhigh=fix(255*colorpairs(:,1+(i-1)*2))';
				p.chromlow=fix(255*colorpairs(:,2+(i-1)*2))';
				newstim = periodicstim(p);
				newstim=recenterstim(newstim,{'rect',cr,...
					'screenrect',sr,'params',1});
	            COLOR_PS= append(COLOR_PS,newstim);
			end;
            COLOR_PS = setDisplayMethod(COLOR_PS,1,vals{1});
            b = transferscripts({'COLOR_PS'},{COLOR_PS});
            if b,
               dowait(0.5);
               b=runscriptremote('COLOR_PS');
               if ~b,
                  errordlg('Could not run script--check RunExperiment window.');
               end;
               tn = get(ft(geteditor('RunExperiment'),'SaveDirEdit'),'String');
               set(ft(thefig,teStr),'String',tn);
               set(ft(thefig,cbStr),'value',0);
               lgnctxexperpanel(cbStr,thefig);
            end;
          end;
       ...
       if get(ft(thefig,'OTPrefCB'),'value')~=1,
          ...
          if b,
            ...
            if b,
               dowait(0.5);
			   z = geteditor('RunExperiment');
			   if ~isempty(z)&get(ft(thefig,'RunSeriesTestCB'),'value'),
				   cmdstr = {'igorseriestest()'};
               	   set(ft(z,'extdevlist'),'string',cmdstr,'value',1:length(cmdstr));
			   else, set(ft(z,'extdevlist'),'string',{},'value',[]);
			   end;
    ...
    case {'ColorSelectRunBt','BlueConeRunBt','GreenConeRunBt','RodRunBt'},
	   cases = {'ColorSelectRunBt','BlueConeRunBt','GreenConeRunBt','RodRunBt'};
	   testr= {'ColorSelectTestEdit','BlueConeTestEdit','GreenConeTestEdit','RodTestEdit'};
	   cbstr= {'ColorSelectCB','BlueConeCB','GreenConeCB','RodCB'};
	   for ii=1:length(cases),if strcmp(thetag,cases{ii}), break; end; end;
       if get(ft(thefig,'OTPrefCB'),'value')~=1,
          errordlg('Cannot run because OT response check box not checked.');
       else, 
          ...
          if b,
            p = getparameters(ud.PS);
			testvals={'ColorSelectRangeEdit','BlueConeRangeEdit','GreenConeRangeEdit','RodRangeEdit'};
			valstr = get(ft(thefig,testvals{ii}),'String');
			try, valval = eval(valstr); p.chromhigh = valval(1,:); p.chromlow=valval(2,:);
			catch, errordlg(['Syntax error in color values: ' valstr]); return;
			end;
            p.dispprefs = {'BGpretime',vals{2}};
            p.sFrequency = vals{3};
            LGNP_PS = periodicscript(p);
            ...
            if b,
               ...
               tn = get(ft(geteditor('RunExperiment'),'SaveDirEdit'),'String');
               set(ft(thefig,testr{ii}),'String',tn);
               set(ft(thefig,cbstr{ii}),'value',0);
               lgnctxexperpanel(cbstr{ii},thefig);
            end;
          end;
       end;
    case 'TFRunBt',     
    ...
    case 'OTRunBt',     
          ...
          if b,
			...
			if get(ft(thefig,'CurrStepsCB'),'value'),
				p.dispprefs={'BGpretime',vals{2},'BGposttime',6}, 
			end;
            LGNP_PS = periodicscript(p);
			[cr,dist,sr] = getscreentoolparams;
			LGNP_PS = recenterstim(LGNP_PS,...
				{'rect',cr,'screenrect',sr,'params',1});
			if get(ft(thefig,'CurrStepsCB'),'value'),
				stimlength=duration(get(LGNP_PS,1))-5; % assume all are same
				z = geteditor('RunExperiment');
				if isempty(z),
				...
				cmdstr = {['igorcurrentsteps(' mat2str(vals{4}) ',' mat2str(do) ...
					',' mat2str(stimlength) ')']};
				if get(ft(thefig,'RunSeriesTestCB'),'value'),
					cmdstr = {'igorseriestest()';cmdstr{1}};
				end;
				set(ft(z,'extdevlist'),'string',cmdstr,'value',1:length(cmdstr));
			else,
				LGNP_PS = setDisplayMethod(LGNP_PS,1,vals{1});
				...
				if get(ft(thefig,'RunSeriesTestCB'),'value'),
					...
					set(ft(z,'extdevlist'),'string',{},'value',[]);
				end;
			end;
            b = transferscripts({'LGNP_PS'},{LGNP_PS});
            if b,
...
PSp = struct('imageType',2,'animType',4,'flickerType',0,'angle',[0],...
             ...
             'rect',[0 0 640 480],'nCycles',5,'contrast',0.8,...
...
set(ft(h1,'CurrStepsEdit'),'string','[400 -400 -1000]');

set(ft(h1,'SusTransLGNRepsEdit'),'String','10');
set(ft(h1,'SusTransLGNISIEdit'),'String','0.5');

set(ft(h1,'GratingRepsEdit'),'String','7');
set(ft(h1,'GratingISIEdit'),'String','2');
set(ft(h1,'OTRangeEdit'),'String','[0:30:330]');
set(ft(h1,'TFRangeEdit'),'String','[1 2 4 8 16]');
set(ft(h1,'SFRangeEdit'),'String','[0.05 0.1 0.2 0.4 0.6 0.8]');
set(ft(h1,'ContrastRangeEdit'),'String','[0.08 0.16 0.32 0.64 0.8 1]');
