function output=mr_plotsftftuning(cellinfo)
% MR_PLOTSFTUNING - plot spatial frequency and temporal frequency tuning information from cells
%
%  OUTPUT = MR_PLOTSFTFTUNING(CELLINFO)
%
%  Plot CELLINFO that is returned from MR_DONORMSURRANALYSIS
%
% 

 % right now only divides cells into rearing types

struct2var(mr_definestudygroups(cellinfo)); % define variables for study groups
...
xvalues = { [0.02 0.05 0.1 0.2 0.5 0.8] , [0.5 1 2 4 6 8 12] };
...
for g = 1:length(F_strings),
	...
	for p = 1:P,
		...
		for i=1:length(myinds{p}),
			...
			if ~isempty(myfield.sig_vis),
				if (myfield.sig_vis< pvalue_criterion) & ( max(myresp(2,:)) >  max_firing_rate_criterion),
		...
		for i=1:length(mygoodcells),
			...
			if myresp(1,1) == 0.025, myresp(1,1) = 0.02; end; % small edit
			[dummy,matchingindexes] = intersect(xvalues{g},myresp(1,:));
			f_curves_ind_norm{p}(end+1,matchingindexes) = myresp(2,matchingindexes) / max(myresp(2,matchingindexes));
	...
	for p=1:P,
		...
		plot(p*ones(size(f_curve_peak{p})),f_curve_peak{p},'go');
	...
	box off;
	ylabel('Firing rate');
	if P~=6, error(['need to update this.']); end;
	set(gca,'xticklabelrotation',45,'xtick',[1:6], 'xticklabels',{'WT TR','WT 2MD','KO TR','KO 2MD','WT P34','KO P34'});
...
if 1,
	...
	blank_stderr = [];
	for p=1:P,
		blank_mns(p) = nanmean(f_curve_blank{p});
		plot(p*ones(size(f_curve_blank{p})),f_curve_blank{p},'go');
		blank_stderr(p) = nanstderr(f_curve_blank{p});
	end;
	h=bar([1:P],blank_mns);
	set(h,'facecolor',[0 0 0]);
	h2 = myerrorbar([1:P],blank_mns,blank_stderr,blank_stderr);
	set(h2,'color',[0 0 0],'linewidth',1);
	delete(h2(2));
	movetoback(h);
	hold on;
	box off;
	ylabel('Blank firing rate');
	set(gca,'xticklabelrotation',45,'xtick',[1:6], 'xticklabels',{'WT TR','WT 2MD','KO TR','KO 2MD','WT P34','KO P34'});
end
