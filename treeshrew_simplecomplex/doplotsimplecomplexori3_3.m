...
l = {};
l{1} = intersect(lll{1},ll{1});
l{2} = intersect(lll{2},ll{2});
l{3} = intersect(lll{3},ll{3});
...
for i=1:3,
    subplot(2,2,i); 
    plot(x_mod(l{i}),x_overlap(l{i}),'k.');
...
syms = {'-','--','-'};
labs = {'Modulation','ON/OFF','ON/OFF seg'};


figure;
for i=1:3, for j=1:3,
    subplot(2,2,i);
    if i==1, d = x_mod; yd = y_mod; elseif i==2, d = 2*abs(x_onoff-0.5); yd = y_onoff; elseif i==3, d = x_overlap; yd = y_overlap; end;
    l = {};
