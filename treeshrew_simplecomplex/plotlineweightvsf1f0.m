...
for i=1:length(cells),
    ...
    A=findassociate(cells{i},'SP F0 Ach normalized potential overlap','','');
    ...
    if ~isempty(rd)&~isempty(A)&~isempty(f1f0r),
        if B.data<0.05, 
            f1f0=[f1f0 f1f0r];
            NPotOverlap=[NPotOverlap A.data];
        end
    end;
end;

plot(NPotOverlap, f1f0, 'bo');
[SLOPE,OFFSET,SLOPE_CONFINTERVAL,RESID, RESIDINT, STATS] = quickregression(NPotOverlap',f1f0',.05);
...
box off
axis([0 1 0 1])
xlabel('Normalized Potential Overlap')
ylabel('F1/(F0+F1)')
