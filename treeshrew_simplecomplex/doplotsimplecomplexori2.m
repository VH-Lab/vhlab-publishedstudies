function doplotsimplecomplexori2(prefix,expernames)
...
axis([0 1.2 -0.2 2]);
xlabel('Orientation index'); ylabel('Depth (mm, adjusted)');
title([int2str(length(x)) ' cells in ' int2str(length(expernames)) ' tree shrews.']);

subplot(1,3,2);
[x,y,s]=plotsimplecomplexori(cells,3);
[Yn,Xn] = slidingwindowfunc(y, x, 0, 0.1, 2.0, 0.2,'median',0);
hold on;
plot(Yn,Xn,'-','color',[0.8 0.8 0.8],'linewidth',2);
ch=get(gca,'children'); set(gca,'children',[ch(2:end);ch(1)]);
axis([0 1.2 -0.2 2]);
xlabel('Tuning width/100'); 
title([int2str(length(x)) ' cells in ' int2str(length(expernames)) ' tree shrews.']);

subplot(1,3,3);
[x,y,s]=plotsimplecomplexori(cells,4);
[Yn,Xn] = slidingwindowfunc(y, x, 0, 0.1, 2.0, 0.2,'median',0);
hold on;
plot(Yn,Xn,'-','color',[0.8 0.8 0.8],'linewidth',2);
ch=get(gca,'children'); set(gca,'children',[ch(2:end);ch(1)]);
