...
pthreshold=0.05;colorscale=0;rotate=0;imagesize=[512 512];dirthresh=-100;
dir_assoc='TP Ach OT Carandini Fit Params';
ot_assoc = 'TP Ach OT Fit Pref'; pthreshold_assoc = 'TP Ach OT vec varies p'; 
supressplot=0; transparency = 0;
...
if isa(ds_or_cells,'dirstruct'),
	...
	if length(cells) == 0, cellnames = {};
	else, for i=1:length(cells), cellnames{i} = ''; end;
	end;
end;
...
for i=1:length(cells),
		...
		bl=findassociate(cells{i},bor_assoc,'','');
...
%g = gca;
if exist('ds')==1,
[im,scalebarax,scalebarimg]=pvcontinuousmap(imagesize,1,cells,cellnames(mycells),ot_assoc,'mod(x-45,360)',0:179,~supressplot,'pvalueassoc',pthreshold_assoc,'pvalue',pthreshold,'symbol',symb,'symbolsize',symsize,'colors',cols,...
