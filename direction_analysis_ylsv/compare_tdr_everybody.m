function compare_tdr_everybody(prefix,expnames,trainingdirs, savefilename, age_threshold)  % filename where results should be saved
...
for i=1:length(expnames),
	...
	if ~isempty(pvalasoc),
		...
		if ~isempty(pvalasoc)&pvalasoc.data<1; %0.05,
            for k=[K 5 8 11 12],
                if (k==K)|((k==5)&((~isempty(pvalasocm)&pvalasocm.data>=0*0.05)|(~isempty(pvalasocf)&pvalasocf.data>=0*0.05)))|...
                        ((k==8)&isempty(pvalasocm)&isempty(pvalasocf))|((k==11)&isempty(pvalasocm)&~isempty(pvalasocf))|...
                        ((k==12)&(~isempty(pvalasocm)&pvalasocm.data<(1.05))|((~isempty(pvalasocf)&pvalasocf.data<(1.05)))),
                    if k==12,
		...
		if ~isempty(pvalasocm)&pvalasocm.data<1,% 0.05,
            for k=[2 6 9 13],
                if (k==2)|(k==6&(~isempty(pvalasoc)&pvalasoc.data>=0*.05))|(k==9&isempty(pvalasoc))|...
                        ((k==13)&(~isempty(pvalasoc)&pvalasoc.data<1.05)),
                    if k==13,
		...
		if ~isempty(pvalasocf)&pvalasocf.data<1.05,
            for k=[3 7 10 14],
                if (k==3)|(k==7&(~isempty(pvalasoc)&pvalasoc.data>=0*0.05))|(k==10&~isempty(pvalasoc))|...
                        ((k==14)&(~isempty(pvalasoc)&pvalasoc.data<1.05)),
                    if k==14,
