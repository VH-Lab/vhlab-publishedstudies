function simulateMapStrength3(prefix,expnames, resimulate)
...
for i=1:length(expnames),
	...
	ds = dirstruct(tpf);
    D = dir([getscratchdirectory(ds) filesep '*_cellbs_newsingle.mat']);
    ...
    for j=1:length(D),
        ...
        try,
            load([getpathname(ds) filesep 'analysis' filesep 'trainingdir.mat']);
        catch, trainingdir = NaN;
        end;
        ...
        if eyesopen<=3,
            TD = cat(2,TD,trainingdir*ones(1,length(mycellstruct)));
            if d1==0,
                mapnaive = mycellstruct; d1 = 1;
            else,
                mapnaive = cat(2,mapnaive,mycellstruct);
            end;
        else,    
            if d2==0,
                mapold = mycellstruct; d2 = 1; 
            else,
                mapold = cat(2,mapold,mycellstruct);
            end;
        end;
    end;
end;
...
if resimulate,
    ...
    save('e:\svanhooser\newferretmtg\mcMapSim_newsingle.mat','mcsim','mcsimt','mcsimo','mapnaive','mapold','mcsimf');
else,
    load('e:\svanhooser\newferretmtg\mcMapSim_newsingle.mat','mcsim','mcsimt','mcsimo','mapnaive','mapold','mcsimf');
end;
...
for i=1:length(mapnaive),
    ...
    mpb(i) = dpmatch(squeeze(mcsim(2,i,:)),TD(i));
    distdb(i) = std(mcsim(1,i,:));
    distmnb(i) = mean(mcsim(1,i,:));
    dib(i,1:3) = prctile(mcsim(1,i,:),[33 50 66]);
    dia(i,1:3) = prctile(mcsimt(1,i,:),[33 50 66]);
    sdib(i,1:3) = prctile(mcsim(1,i,:).*(-1 * double(angdiff(TD(i)-mcsim(2,i,:))>90) + double(angdiff(TD(i)-mcsim(2,i,:))<=90)),[33 50 66]);
    sdia(i,1:3) = prctile(mcsim(1,i,:).*(-1 * double(angdiff(TD(i)-mcsimt(2,i,:))>90) + double(angdiff(TD(i)-mcsimt(2,i,:))<=90)),[33 50 66]);
    ...
    lmb{i} = localdirmatch(TD(i),mapnaive(i).pos,reshape(mcsim(2,inds,:),length(inds),100)',xyothers, dists,1,0);
    lmb2{i} = localdirmatch(TD(i),mapnaive(i).pos,reshape(mcsim(2,inds,:),length(inds),100)',xyothers, dists,1,1);
    ...
    for k=1:length(lbb{i}),
        ...
        lmbd(i,1:3,k) = prctile(lmb{i}{k},[33 50 66]);
        lmbd2(i,1:3,k) = prctile(lmb2{i}{k},[33 50 66]);
        lmbd_(i,1,k) = nanmean(lmb{i}{k});
        lmbd2_(i,1,k) = nanmean(lmb2{i}{k});        
    ...
    if ~isnan(mcsimt(2,i,1)),
        ...
        mpa(i) = dpmatch(squeeze(mcsimt(2,i,:)),TD(i));
        ...
        lma{i} = localdirmatch(TD(i),mapnaive(i).pos,reshape(mcsimt(2,inds,:),length(inds),100)',xyothers, dists,1,0);
        lma2{i} = localdirmatch(TD(i),mapnaive(i).pos,reshape(mcsimt(2,inds,:),length(inds),100)',xyothers, dists,1,1);
        ...
        distmna(i) = NaN;
        ddiLB{i} = NaN;
        laoba{i} = mat2cell(repmat(NaN,1,length(dists)),1,repmat(1,1,length(dists)));
        laosba{i} = mat2cell(repmat(NaN,1,length(dists)),1,repmat(1,1,length(dists)));
        ladba{i} = mat2cell(repmat(NaN,1,length(dists)),1,repmat(1,1,length(dists)));
        ladsba{i} = mat2cell(repmat(NaN,1,length(dists)),1,repmat(1,1,length(dists)));
        lba{i} = mat2cell(repmat(NaN,1,length(dists)),1,repmat(1,1,length(dists)));
        lma{i} = mat2cell(repmat(NaN,1,length(dists)),1,repmat(1,1,length(dists)));
        lma2{i} = mat2cell(repmat(NaN,1,length(dists)),1,repmat(1,1,length(dists)));
        lsa{i} = mat2cell(repmat(NaN,1,length(dists)),1,repmat(1,1,length(dists)));
        loa{i} = mat2cell(repmat(NaN,1,length(dists)),1,repmat(1,1,length(dists)));
        lsba{i} = mat2cell(repmat(NaN,1,length(dists)),1,repmat(1,1,length(dists)));
        loba{i} = mat2cell(repmat(NaN,1,length(dists)),1,repmat(1,1,length(dists)));
        ldsba{i} = mat2cell(repmat(NaN,1,length(dists)),1,repmat(1,1,length(dists)));
    end;
    if ~isnan(mcsimf(2,i,1)),
        ...
        unclistf(i) = NaN;
        ddiLBf{i} = NaN;
        laobaf{i} = mat2cell(repmat(NaN,1,length(dists)),1,repmat(1,1,length(dists)));
        laosbaf{i} = mat2cell(repmat(NaN,1,length(dists)),1,repmat(1,1,length(dists)));
        ladbaf{i} = mat2cell(repmat(NaN,1,length(dists)),1,repmat(1,1,length(dists)));
        ladsbaf{i} =mat2cell(repmat(NaN,1,length(dists)),1,repmat(1,1,length(dists)));
        lbaf{i} = mat2cell(repmat(NaN,1,length(dists)),1,repmat(1,1,length(dists)));
        lsaf{i} = mat2cell(repmat(NaN,1,length(dists)),1,repmat(1,1,length(dists)));
        loaf{i} = mat2cell(repmat(NaN,1,length(dists)),1,repmat(1,1,length(dists)));
        lsbaf{i} = mat2cell(repmat(NaN,1,length(dists)),1,repmat(1,1,length(dists)));
        lobaf{i} = mat2cell(repmat(NaN,1,length(dists)),1,repmat(1,1,length(dists)));
        ldsbaf{i} = mat2cell(repmat(NaN,1,length(dists)),1,repmat(1,1,length(dists)));
    end;
    for k=1:length(lba{i}),
        ...
        lmad(i,1:3,k) = prctile(lma{i}{k},[33 50 66]);
        lmad2(i,1:3,k) = prctile(lma2{i}{k},[33 50 66]);        
        lmad_(i,1,k) = nanmean(lma{i}{k});
        lmad2_(i,1,k) = nanmean(lma2{i}{k});
...
save('e:\svanhooser\newferretmtg\mcMapSimCompute_2010singlefull.mat');
