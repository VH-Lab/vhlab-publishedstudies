function [lb] = localdirbiasNEW(dp,myxy,dpothers,xyothers, dists, dirorori, annuli)

%see also localdirbiasNEWMC--MC, use cosine to calculate angle difference between target cell and surrounding cells
...
for j=1:length(dists),
    ...
    if length(ind)>=1,
        ...
        for k=1:length(dp),
            ...
            %lb{j}(k) = (length(find(mydiff<45))-length(find(mydiff>(90+45))))/(length(ind)-sum(isnan(mydiff)))+0.3;
