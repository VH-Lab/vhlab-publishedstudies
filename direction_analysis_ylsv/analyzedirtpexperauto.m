...
r='y';%r = input('Do you want to calculate/recalculate cell fits? (y/n)','s');
...
if strcmp(r,'y')|strcmp(r,'Y'),
  for i=1:length(cells),
	...
	cells{i} = tpotanalysis(ds,cells{i},cellnames{i},plotit);
  end;
end;

dirtpexpsummary(cells,cellnames);

r='y'; % r = input('Do you want to plot all stacks? (y/n)','s');
if strcmp(r,'y')|strcmp(r,'Y'),
	makedirexpsummaryplots(cells,ds);
	if ~isempty(ds), makepreviewtpplots(ds); end;
end;

r='y';%r = input('Do you want to save the cells back to the database? (y/n)','s');
