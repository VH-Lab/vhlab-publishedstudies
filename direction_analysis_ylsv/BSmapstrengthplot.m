...
% mpbUNall = []; mpaUNall = []; BSlmib100UNall = []; BSlmia100UNall = [];
 mpball = []; mpaall = []; BSlmib100all = []; BSlmia100all = [];
...
for I = 1: length(expname),
 ...
 figure;
 subplot(1,3,1);plot(BSlmib100(indPT), mpa(indPT)-mpb(indPT),'ro');axis([-1 1 -1 1]); axis square; title (expname(I));
 subplot(1,3,2);plot(BSlmib100(indNT), mpa(indNT)-mpb(indNT),'go');axis([-1 1 -1 1]); axis square; title (expname(I));
 subplot(1,3,3);plot(BSlmib100(indUN), mpa(indUN)-mpb(indUN),'ko');axis([-1 1 -1 1]); axis square; title (expname(I));
...
mpball = [mpball mpb]; mpaall = [mpaall mpa]; BSlmib100all = [BSlmib100all BSlmib100]; BSlmia100all = [BSlmia100all BSlmia100];
...
stepsize = 0.1;
figure;
subplot(1,3,1);plot(BSlmib100all(indPT), mpaall(indPT)-mpball(indPT),'ro');axis([-.6 .6 -1 1]); axis square; title('prefer')
[Yn,Xn,Yerr_p] = slidingwindowfunc(BSlmib100all(indPT), mpaall(indPT)-mpball(indPT), -1, stepsize, 1, 0.2, 'nanmean',0,'prctile(y,[50])');
hold on;
%plot(Xn-.2,Yn,'k-','linewidth',2); box off;
subplot(1,3,2);plot(BSlmib100all(indNT), mpaall(indNT)-mpball(indNT),'go','color',[0 0 1]);axis([-.6 .6 -1 1]); axis square; title('non-prefer')
[Yn,Xn,Yerr_p] = slidingwindowfunc(BSlmib100all(indNT), mpaall(indNT)-mpball(indNT), -1, stepsize, 1, 0.2, 'nanmean',0,'prctile(y,[50])');
hold on;
plot(Xn,Yn,'k-','linewidth',2); box off;
subplot(1,3,3);plot(BSlmib100all(indUN), mpaall(indUN)-mpball(indUN),'o','color',[0.5 0.5 0.5]);axis([-.6 .6 -1 1]); axis square; title('uncertain')
[Yn,Xn,Yerr_p] = slidingwindowfunc(BSlmib100all(indUN), mpaall(indUN)-mpball(indUN), -1, stepsize, 1, 0.2, 'nanmean',0,'prctile(y,[50])');
hold on;
%plot(Xn-.2,Yn,'k-','linewidth',2); box off;
