...
for i=1:length(mapnaive),
    ...
    myinds = find(dists<=(100*512/240)&~isnan(squeeze(mcsimt(2,inds,1))'));
    ...
    if ~isnan(mcsimt(2,i,1)),
        ...
        [unai,dummy,dumy,medangai] = dpuncertainty(squeeze(mcsimt(2,i,:)));
        [psamebi,pdiffbi,pdifforibi] = dpuncertaintytot(squeeze(mcsim(2,i,:)));
        [psameai,pdiffai,pdifforiai] = dpuncertaintytot(squeeze(mcsimt(2,i,:)), medangbi);
        ...
        for j=1:length(myinds),
            ...
            [unaj,dummy,dumy,medangaj] = dpuncertainty(squeeze(mcsimt(2,inds(myinds(j)),:)));
            [psamebj,pdiffbj,pdifforibj] = dpuncertaintytot(squeeze(mcsim(2,inds(myinds(j)),:)), medangbi);
            [psameaj,pdiffaj,pdifforiaj] = dpuncertaintytot(squeeze(mcsimt(2,inds(myinds(j)),:)), medangbi);
            [psameaj_,pdiffaj_,pdifforiaj_] = dpuncertaintytot(squeeze(mcsimt(2,inds(myinds(j)),:)), medangai);
            fpi = fp(i); fpj = fp(inds(myinds(j)));
            % old way
            ...
            dLCIest(i) = dLCIest(i) + (E+F-G-H)-((A+C)-(B+D));
            %dLCIest2(i) = dLCIest2(i) + (1-AEfp)*(E-A)+(1-CFfp)*(F-C)-(1-BGfp)*(G-B)-(1-DHfp)*(H-D); % terms without flipping
            %dLCIest3(i) = dLCIest3(i) + (AEfp*(E-A)+CFfp*(F-C)-BGfp*(G-B)-DHfp*(H-D));  % terms with flipping
    ...
    dLCIact(i) = lbad(i,2,2) - lbbd(i,2,2);
end;
...
G = find(~isnan(dLCIact));
D=dLCIact(G)-dLCIest2(G)-dLCIest3(G); DM = dLCIact(G);
sum(D.^2)/sum(DM.^2)
