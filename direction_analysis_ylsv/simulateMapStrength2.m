function simulateMapStrength2(prefix,expnames, resimulate)
...
for i=1:length(expnames),
    ...
    for j=1:length(D),
        ...
        if eyesopen<=3,
            if d1==0, mapnaive = mycellstruct; d1 = 1;
            else, mapnaive = cat(2,mapnaive,mycellstruct); end;
        else,    
...
for i=1:length(mapnaive),
    ...
    lmb{i} = localdirmatch(TRAINDIR,mapnaive(i).pos,reshape(mcsim(2,inds,:),length(inds),100)',xyothers, dists,1,0);
    ...
    if ~isnan(mcsimt(2,i,1)),
        ...
        lma{i} = localdirmatch(mcsimt(2,i,:),mapnaive(i).pos,reshape(mcsimt(2,inds,:),length(inds),100)',xyothers, dists,1,0);
