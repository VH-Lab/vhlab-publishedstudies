...
load('/Volumes/stevedata/svanhooser/newferretmtg/mcMapSimCompute_2010singlefull.mat');
%load('e:\svanhooser\newferretmtg\mcMapSimCompute_2010singlefull.mat');
%load('e:\mchristensson\matlab\mcMapSimCompute_single.mat');
...
%keyboard;
max(mpb),min(mpb),
ind{1} = find(mpb>=0.75&~isnan(mpa));
ind{2} = find(mpb<=0.25&~isnan(mpa));
ind{3} = find(mpb<0.75&mpb>0.25&~isnan(mpa));
ind{4} = find(~isnan(mpb));
ind{5} = find(~isnan(mpa)&~isnan(mpb));
cols = [1 1 0;0 0 1; 0.7 0.7 0.7; 0 0 0 ];
figure;


for i=1:4,
    subplot(2,2,i);
    [Yn,Xn,Yerr_p] = slidingwindowfunc(lmbd(ind{i},2,dist), mpa(ind{i})-mpb(ind{i}), -1-0.2, stepsize, 1+0.2, 0.2, 'nanmedian',0,'prctile(y,[50])');
    plot(lmbd(ind{i},2,dist), mpa(ind{i})-mpb(ind{i}),'o','color',cols(i,:));
    hold on;
    plot(Xn,Yn,'k-','linewidth',2); box off;
    axis([-1 1 -1 1]); axis square
    title(['Local matching before vs. \Delta MP']);
end;

figure;
for i=1:4,
    subplot(2,2,i);
    [Yn,Xn,Yerr_p] = slidingwindowfunc(lmbd(ind{i},2,dist), sdia(ind{i})-sdib(ind{i}), -1-0.2, stepsize, 1+0.2, 0.2, 'nanmean',0,'prctile(y,[50])');
    plot(lmbd(ind{i},2,dist), sdia(ind{i})-sdib(ind{i}),'o','color',cols(i,:));
    hold on;
    plot(Xn,Yn,'k-','linewidth',2); box off;
    title(['Local matching before vs. \Delta sDI']);
    axis([-1 1 -1 1]); axis square
end;

for i=1:4,
    [SLOPE,OFFSET,SLOPE_CONFINTERVAL,RESID, RESIDINT, STATS] = quickregression(lmbd(ind{i},2,dist),[mpa(ind{i})-mpb(ind{i})]', 0.05); SLOPE_CONFINTERVAL,
end; 

for i=1:4,
    [SLOPE,OFFSET,SLOPE_CONFINTERVAL,RESID, RESIDINT, STATS] = quickregression(lmbd(ind{i},2,dist),[sdia(ind{i})-sdib(ind{i})]', 0.05); SLOPE_CONFINTERVAL,
end; 
...
for i=1:4,
    subplot(2,2,i);
    [Yn,Xn,Yerr_p] = slidingwindowfunc(nanmean(lmbd2_(ind{i},1,thedists),3), mpa(ind{i})-mpb(ind{i}), -1-0.2, stepsize, 1+0.2, 0.2, 'nanmedian',0,'prctile(y,[50])');
    plot(nanmean(lmbd2_(ind{i},1,thedists),3), mpa(ind{i})-mpb(ind{i}),'o','color',cols(i,:));
    hold on;
    plot(Xn,Yn,'k-','linewidth',2); box off;
    title('mean of LMb, over all distances vs. \Delta MP');
    axis([-1 1 -1 1]); axis square
...
for i=1:4,
    subplot(2,2,i);
    [Yn,Xn,Yerr_p] = slidingwindowfunc(nanmean(lmbd2_(ind{i},1,thedists),3), sdia(ind{i})-sdib(ind{i}), -1-0.2, stepsize, 1+0.2, 0.2, 'nanmean',0,'prctile(y,[50])');
    plot(nanmean(lmbd2_(ind{i},1,thedists),3), sdia(ind{i})-sdib(ind{i}),'o','color',cols(i,:));
    hold on;
    plot(Xn,Yn,'k-','linewidth',2); box off;
    axis([-1 1 -1 1]); axis square
    title('mean of LMb, over all distances vs. \Delta sDI');
end;


figure;

[X1,Y1]=cumhist(mpb(ind{5}),[0 1],1);
[X2,Y2]=cumhist(mpa(ind{5}),[0 1],1);
plot(X1,Y1,'g','linewidth',2); box off;
hold on;
...
figure;
[X1,Y1]=cumhist(dib(ind{5}),[0 1],1);
[X2,Y2]=cumhist(dia(ind{5}),[0 1],1);
plot(X1,Y1,'g','linewidth',2); box off;
hold on;
plot(X2,Y2,'r','linewidth',2);
axis([0 1 0 100]);
...
dmp = (mpa-mpb).*1./( (double(mpa-mpb>0).*(1-mpb)) + double(mpa-mpb<=0).*mpb);

figure;
for i=1:4,
    subplot(2,2,i);
    [Yn,Xn,Yerr_p] = slidingwindowfunc(lmbd(ind{i},2,dist), dmp(ind{i}), -1-0.2, stepsize, 1+0.2, 0.2, 'nanmedian',0,'prctile(y,[50])');
    plot(lmbd(ind{i},2,dist), dmp(ind{i}),'o','color',cols(i,:));
    hold on;
    plot(Xn,Yn,'k-','linewidth',2); box off;
end;
...
for i=1:4,
    subplot(2,2,i);
    [Yn,Xn,Yerr_p] = slidingwindowfunc(lmbd(ind{i},2,dist), sdia(ind{i})-sdib(ind{i}), -1-0.2, stepsize, 1+0.2, 0.2, 'nanmean',0,'prctile(y,[50])');
    plot(lmbd2(ind{i},2,dist), sdia(ind{i})-sdib(ind{i}),'o','color',cols(i,:));
    hold on;
    plot(Xn,Yn,'k-','linewidth',2); box off;
end;
end;
...
for i=1:4,
    subplot(2,2,i);
    [Yn,Xn,Yerr_p] = slidingwindowfunc(nanmean(lmbd2(ind{i},1,thedists),3), mpa(ind{i})-mpb(ind{i}), -1-0.2, stepsize, 1+0.2, 0.2, 'nanmedian',0,'prctile(y,[50])');
    plot(nanmean(lmbd2(ind{i},1,thedists),3), mpa(ind{i})-mpb(ind{i}),'o','color',cols(i,:));
    hold on;
    plot(Xn,Yn,'k-','linewidth',2); box off;
end;
...
for i=1:4,
    subplot(2,2,i);
    [Yn,Xn,Yerr_p] = slidingwindowfunc(nanmean(lmbd2(ind{i},1,thedists),3), sdia(ind{i})-sdib(ind{i}), -1-0.2, stepsize, 1+0.2, 0.2, 'nanmean',0,'prctile(y,[50])');
    plot(nanmean(lmbd2(ind{i},1,thedists),3), sdia(ind{i})-sdib(ind{i}),'o','color',cols(i,:));
    hold on;
    plot(Xn,Yn,'k-','linewidth',2); box off;
end;
