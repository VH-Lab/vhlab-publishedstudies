function plotSimMapStrength(expname, traindir)
...
for i=1:length(mapnaive),
    changeDI(i) = mean(mcsimt(1,i,:) -  mcsim(1,i,:));
    I = strmatch({mapnaive(i).expername},expnames,'exact'),
    diffs = angdiff(mcsim(2,i,:)-traindir(I));
    numsame = length(find(diffs<45));
    numdiff = length(find(diffs>135));
    if numsame>75, sameastraining(end+1) = i; end;
    if numdiff>75, difffromtraining(end+1) = i; end;
end;

hold on;
plot(100*lbbd(sameastraining,2,2),fp(sameastraining),'ks')
[slope,offset,slope_confinterval,resid,residint,stats] = quickregression(100*lbbd(sameastraining,2,3),fp(sameastraining)',0.05);
slope_confinterval,
...
plot(100*lbbd(difffromtraining,2,2),fp(difffromtraining),'ko')
[slope,offset,slope_confinterval,resid,residint,stats] = quickregression(100*lbbd(difffromtraining,2,3),fp(difffromtraining)',0.05);
slope_confinterval,
h=plot([-100 100],[-100 100]*slope+offset,'k--'); set(h,'color',[1 1 1]*0)
axis ([ -60 60 0 1.0]);
ylabel('Prob of Dir Flip'); xlabel('Local bias before training');
...
figure;
%changeDI here
  % change 'fp' to changeDI
  % change ylabels
hold on;
plot(100*lbbd(sameastraining,2,2),changeDI(sameastraining),'ks')
[slope,offset,slope_confinterval,resid,residint,stats] = quickregression(100*lbbd(sameastraining,2,3),changeDI(sameastraining)',0.05);
slope_confinterval,
h=plot([-100 100],[-100 100]*slope+offset,'k-'); set(h,'color',[1 1 1]*0)
axis ([ -60 60 0 1.0]);
...
plot(100*lbbd(difffromtraining,2,2),changeDI(difffromtraining),'ko')
[slope,offset,slope_confinterval,resid,residint,stats] = quickregression(100*lbbd(difffromtraining,2,3),changeDI(difffromtraining)',0.05);
slope_confinterval,
h=plot([-100 100],[-100 100]*slope+offset,'k--'); set(h,'color',[1 1 1]*0)
axis ([ -60 60 0 1.0]);
ylabel('Change in DI'); xlabel('Local bias before training');
...
% plot([0 300],[0 0],'k--');
% 
% l=legend('Motion training','Naive');
% set(l,'box','off','position',[0.6116    0.7283    0.2554    0.1444]);
% box off;
% ch = get(gca,'children'); set(gca,'children',[ch(3:end); ch(1); ch(2)]);
% xlabel('Cell distance (\mum)');
