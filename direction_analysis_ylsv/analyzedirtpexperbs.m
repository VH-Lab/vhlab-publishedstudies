function [cells,cellnames]=analyzedirtpexperbs(arg1,arg2,arg3)
...
if strcmp(r,'y')|strcmp(r,'Y'),
  ...
  d=dir([fixpath(sd) 'bsprog.mat']);
  if ~isempty(d),
      listofnames = sort({d.name});
      myfile = load([fixpath(sd) 'bsprog.mat']);
      cells = myfile.cells; cellnames = myfile.cellnames; i=myfile.i;
  else,
      i = 1;
  end;
  ...
  while i<=length(cells),
	...
	cells{i} = tpgenericanalysis(ds,cells{i},cellnames{i},plotit,{'Best orientation test'},'p.angle','otfitbootstrap','TP','plottpresponse(newcell,cellname,''Orientation/Direction'',1,1,''usesig'',1)');	
	cells{i} = tpgenericanalysis(ds,cells{i},cellnames{i},plotit,{'Best OT recovery test'},'p.angle','otfitbootstrap','TP ME','plottpresponse(newcell,cellname,''Orientation/Direction ME'',1,1,''usesig'',1)');
	cells{i} = tpgenericanalysis(ds,cells{i},cellnames{i},plotit,{'Best Flash OT recovery test'},'p.angle','otfitbootstrap','TP FE','plottpresponse(newcell,cellname,''Orientation/Direction FE'',1,1,''usesig'',1)');

    cells{i} = tpdirtrainsig(ds,cells{i},cellnames{i},plotit);
	[cellnames{i} ', ' int2str(i) ],
    if mod(i,5)==0|i==length(cells), disp(['Saving progress...' int2str(i) ' of ' int2str(length(cells)) '.']); save([fixpath(sd) 'bsprog' sprintf('%0.5',i) '.mat'],'cells','cellnames','i'); end;
    i = i + 1;
  end;
end;
