...
for I=1:length(expnames),
	...
	for J=1:length(stacknames),
		...
		pixellocs = {};
		pixellocst = {};
        mycellstruct = struct('DI',[],'BSf',[],'BSrs',[],'DIme',[],'BSfme','','BSrsme',[],'DIfe',[],'BSffe',[],'BSrsfe',[],'pos',[],'expername','','stack','');
		...
		for z=1:length(mycells),
			...
			if ~isempty(pvalasoc),
				if pvalasoc.data<0.05,
                    ...
                    mycs = struct('DI',di(end),'BSf',BSf,'BSrs',BSrs,'DIme',[],'BSfme',[],'BSrsme',[],'DIfe',[],'BSffe',[],'BSrsfe',[],'pos',[mean(pixellocs{end}.x) mean(pixellocs{end}.y)],'expername',expnames{I},'stack',stacknames{J});
	    ...
	    save([getscratchdirectory(ds) filesep stacknames{J} '_cellbs.mat'],'mycellstruct','eyesopen');
	end;
end;