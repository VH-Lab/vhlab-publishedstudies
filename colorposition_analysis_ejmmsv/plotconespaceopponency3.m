...
for I=1:length(V),
    for J=1:length(V),
        for k=1:length(V),
            MAT(I,J,k) = length(find(WM(inds)==4&COG(inds)==V(k)&CAG(inds)==V(J)&SOG(inds)==V(I)));
            MAT2(I,J,k) = length(find(WM(inds)==1&COG(inds)==V(k)&CAG(inds)==V(J)&SOG(inds)==V(I)));
            MAT3(I,J,k) = length(find(WM(inds)==2&COG(inds)==V(k)&CAG(inds)==V(J)&SOG(inds)==V(I)));            
        end;
    end;
end;
...
for I=1:length(S),
    for J=1:length(S),
        for k=1:length(S),
            for z=1:length(S),
                state = [ S(I) S(J) S(k) S(z) ];
                COG__(end+1) = double((state(1)==1&state(2)==-1)|(state(1)==-1&state(2)==1)) + double((state(3)==1&state(4)==-1)|(state(3)==-1&state(4)==1));
                CAG__(end+1) = double((state(1)==1&state(4)==-1)|(state(1)==-1&state(4)==1)) + double((state(3)==1&state(2)==-1)|(state(3)==-1&state(2)==1));
                SOG__(end+1) = double((state(1)==1&state(3)==-1)|(state(1)==-1&state(3)==1)) + double((state(2)==1&state(4)==-1)|(state(2)==-1&state(4)==1));
            end;
        end;
    end;
end;
...
for I=1:length(V),
    for J=1:length(V),
        for k=1:length(V),
            MATP(I,J,k) = length(find(COG__==V(k)&CAG__==V(J)&SOG__==V(I)));
            labels{I,J,k} = [int2str(V(k))  int2str(V(J)) int2str(V(I)) ];
        end;
    end;
end;
...
for I=1:length(V),
    for J=1:length(V),
        MAT(I,J) = length(find(COG(inds)==V(I)&SOG(inds)==V(J)));
    end;
end;
100*MAT/length(inds),

(sum(COGe(inds)<0)+sum(COGi(inds)<0))/sum(2*imag(COG(inds)))
(sum(COGe(inds)>0)+sum(COGi(inds)>0))/sum(2*imag(COG(inds)))
sum(COGl(inds))/sum(2*imag(SOG(inds)))
sum(COGs(inds))/sum(2*imag(SOG(inds)))
...
myinds = find(COG(inds)==0&SOG(inds)==0&CAG(inds)==0);
O{1,1} = OI(inds(myinds)); Os{1,1} = OIs(inds(myinds));
O{2,1} = OT(inds(myinds)); Os{2,1} = OTs(inds(myinds));
O{3,1} = CV(inds(myinds)); Os{3,1} = CVs(inds(myinds));

myinds = find(COG(inds)==0&SOG(inds)==2&CAG(inds)==2);
O{1,2} = OI(inds(myinds)); Os{1,2} = OIs(inds(myinds));
O{2,2} = OT(inds(myinds)); Os{2,2} = OTs(inds(myinds));
O{3,2} = CV(inds(myinds)); Os{3,2} = CVs(inds(myinds));

myinds = find(COG(inds)==01&SOG(inds)==1&CAG(inds)==0);
O{1,3} = OI(inds(myinds)); Os{1,3} = OIs(inds(myinds));
O{2,3} = OT(inds(myinds)); Os{2,3} = OTs(inds(myinds));
O{3,3} = CV(inds(myinds)); Os{3,3} = CVs(inds(myinds));

myinds = find(COG(inds)==2&SOG(inds)==2&CAG(inds)==0);
O{1,4} = OI(inds(myinds)); Os{1,4} = OIs(inds(myinds));
O{2,4} = OT(inds(myinds)); Os{2,4} = OTs(inds(myinds));
O{3,4} = CV(inds(myinds)); Os{3,4} = CVs(inds(myinds));

for j=1:4, for i=1:3, O{i,j} = O{i,j}(find(~isnan(O{i,j}))); Os{i,j} = Os{i,j}(find(~isnan(Os{i,j}))); end; end;
...
for k=1:2,
    ...
    for i=1:3,
        for j=1:4,
            Xh = X(j)+0.2*(rand(size(O{i,j}))-0.5)
            X_ = [X_ Xh];
            
            Y_ = [Y_; rescale(O{i,j}',xc(i,:),yc(i,:))];
            plot(Xh,rescale(O{i,j}',xc(i,:),yc(i,:)),'k.'); hold on; 
        end;
    end;
	box off; 
    plot([1.5 1.5],[3.5 4.5],'k-');
    plot([1.5 1.5],[1.5 3],'k-');
    plot([1.5 1.5],[0 1],'k-');
end;

INDS=[9 62 81];
INDS = [13 29 110 7 67]
%for i=1:length(INDS), subplot(3,3,i); plottpresponse(g.cellarch{g.myinds(INDS(i))},'','Color Dacey Fig Plot',1,1,'subtractblank',1,'plotblankrange',1); end;
for i=1:length(INDS), subplot(3,3,i); plottpresponse(g.cellarch{g.myinds(INDS(i))},'','Orientation/Direction',1,1,'subtractblank',1,'plotblankrange',1); plot([175 175],[0 0.1],'k-'); A=axis;axis([-5 180 A(3) A(4)]);
g.cellnamearch(INDS),
end;
