...
types = {'Best OT recovery test','Best X pos recovery test','Best Y pos recovery test',...
    ...
    'Contral eye OT test','Ipsi eye OT test','Contral eye OT recovery test','Ipsi eye OT recovery test'};
...
for i=1:length(types),
	if strcmp(type,types{i}),
		...
		elseif strcmp(type,'Best Flash OT recovery test'),
			[is_type,must_be_unique,must_ask,replace_existing]=ismytestdir(sswhatvaries(script),'angle',1,1,1);	
        elseif strcmp(type,'Contral eye OT test'),
			[is_type,must_be_unique,must_ask,replace_existing]=ismytestdir(sswhatvaries(script),'angle',1,1,1);	
        elseif strcmp(type,'Ipsi eye OT test'),
			[is_type,must_be_unique,must_ask,replace_existing]=ismytestdir(sswhatvaries(script),'angle',1,1,1);	
        elseif strcmp(type,'Contral eye OT recovery test'),
			[is_type,must_be_unique,must_ask,replace_existing]=ismytestdir(sswhatvaries(script),'angle',1,1,1);	
        elseif strcmp(type,'Ipsi eye OT recovery test'),
			[is_type,must_be_unique,must_ask,replace_existing]=ismytestdir(sswhatvaries(script),'angle',1,1,1);	
        else,
